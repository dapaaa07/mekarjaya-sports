...
@section('title', 'Edit Jadwal Latihan - ' . $schedule->title)
@section('page-header', 'Edit Sesi Latihan SSB')
...
<div class="max-w-2xl mx-auto editorial-card p-8 space-y-6">
    ...
    <div class="flex items-center justify-between border-b border-[#ebe7e1] pb-4">
        <div>
            <h2 class="font-bold text-[#111111] text-base">Edit Sesi Latihan: {{ $schedule->title }}</h2>
            <p class="text-xs text-[#626260]">Tanggal Sesi: <span class="font-mono text-[#ff5600] font-bold">{{ $schedule->schedule_date->format('d M Y') }}</span></p>
        </div>
    ...
    <form action="{{ route('admin.schedules.update', $schedule->id) }}" method="POST" class="space-y-6 text-xs">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
                ...
                <input type="date" id="schedule_date" name="schedule_date" required value="{{ old('schedule_date', $schedule->schedule_date->format('Y-m-d')) }}"
            ...
            <div>
                ...
                <select id="target_ku" name="target_ku" required class="w-full px-3.5 py-2.5 rounded-md border border-[#d3cec6] bg-white text-[#111111] focus:outline-none focus:ring-1 focus:ring-[#111111]">
                    <option value="Semua KU" {{ old('target_ku', $schedule->target_ku) == 'Semua KU' ? 'selected' : '' }}>Semua Kelompok Umur (Semua KU)</option>
                    @foreach($ageCategories as $cat)
                        <option value="{{ $cat->code }}" {{ old('target_ku', $schedule->target_ku) == $cat->code ? 'selected' : '' }}>{{ $cat->code }} ({{ $cat->min_birth_year }}-{{ $cat->max_birth_year }})</option>
                    @endforeach
                </select>
            </div>
            ...
            <div>
                ...
                <input type="time" id="start_time" name="start_time" required value="{{ old('start_time', substr($schedule->start_time, 0, 5)) }}"
            ...
            <div>
                ...
                <input type="time" id="end_time" name="end_time" required value="{{ old('end_time', substr($schedule->end_time, 0, 5)) }}"
                    class="w-full px-3.5 py-2.5 rounded-md border border-[#d3cec6] bg-white text-[#111111] focus:outline-none focus:ring-1 focus:ring-[#111111]">
            </div>

            <div>
                <label for="location" class="block font-medium text-[#626260] uppercase mb-1">Lokasi Sesi Latihan *</label>
                <input type="text" id="location" name="location" required value="{{ old('location', $schedule->location) }}"
                    class="w-full px-3.5 py-2.5 rounded-md border border-[#d3cec6] bg-white text-[#111111] focus:outline-none focus:ring-1 focus:ring-[#111111]">
            </div>
            ...
            <div>
                ...
                <select id="coach_in_charge" name="coach_in_charge" class="w-full px-3.5 py-2.5 rounded-md border border-[#d3cec6] bg-white text-[#111111] focus:outline-none focus:ring-1 focus:ring-[#111111]">
                    ...
                    @foreach($coaches as $coach)
                        <option value="{{ $coach->name }}" {{ old('coach_in_charge', $schedule->coach_in_charge) == $coach->name ? 'selected' : '' }}>{{ $coach->name }} ({{ $coach->license }})</option>
                    @endforeach
                </select>
            </div>
            ...
            <div>
                ...
                <select id="status" name="status" required class="w-full px-3.5 py-2.5 rounded-md border border-[#d3cec6] bg-white text-[#111111] focus:outline-none focus:ring-1 focus:ring-[#111111]">
                    <option value="scheduled" {{ old('status', $schedule->status) == 'scheduled' ? 'selected' : '' }}>Terjadwal (Scheduled)</option>
                    <option value="completed" {{ old('status', $schedule->status) == 'completed' ? 'selected' : '' }}>Selesai (Completed)</option>
                    <option value="cancelled" {{ old('status', $schedule->status) == 'cancelled' ? 'selected' : '' }}>Dibatalkan (Cancelled)</option>
                </select>
            </div>
            ...
            <div class="sm:col-span-2">
                ...
                <textarea id="notes" name="notes" rows="3"
                    class="w-full px-3.5 py-2.5 rounded-md border border-[#d3cec6] bg-white text-[#111111] focus:outline-none focus:ring-1 focus:ring-[#111111]">{{ old('notes', $schedule->notes) }}</textarea>
            </div>
